...
function EspObject.new(character, interface)
	local self = setmetatable({}, EspObject)
	self.character = assert(character, "Missing argument #1 (Character expected)")
...
function EspObject:Construct()
	...
	self.drawings = {
		...
		visible = {
			...
			name = self:create("Text", { Text = self.character.Name, Center = true, Visible = false }),
...
function EspObject:Update()
	...
	self.options = interface.teamSettings[player and "friendly" or "enemy"]
	self.health, self.maxHealth = interface.getHealth(self.character)
	self.weapon = interface.getWeapon(self.character)
	self.enabled = self.options.enabled and self.character and not
		(player and #interface.whitelist > 0 and not find(interface.whitelist, player.UserId))
...
function ChamObject.new(character, interface)
...
function ChamObject:Update()
	...
	local interface = self.interface
	local player = players:GetPlayerFromCharacter(self.character)
	local options = interface.teamSettings[player and "friendly" or "enemy"]
	local enabled = options.enabled and self.character and not
		(player and #interface.whitelist > 0 and not find(interface.whitelist, player.UserId))
...
function EspInterface.Load(container:Instance)
	assert(not EspInterface._hasLoaded, "Esp has not been loaded yet.")
	local function createObject(character)
		EspInterface._objectCache[character] = {
			EspObject.new(character, EspInterface),
			ChamObject.new(character, EspInterface)
		}
	end
	local function removeObject(character)
		local object = EspInterface._objectCache[character]
		if object then
			for i = 1, #object do
				object[i]:Destruct()
			end
			EspInterface._objectCache[character] = nil
		end
	end
	for _, character in next, container:GetChildren() do
		if character:IsA("Model") and character ~= localPlayer.Character and findFirstChild(character, "Humanoid") and findFirstChild(character, "Head") then
			createObject(character)
		end
	end
	EspInterface.descendantAdded = workspace.DescendantAdded:Connect(function(descendant)
		if descendant:IsA("Model") and findFirstChild(descendant, "Humanoid") and findFirstChild(descendant, "Head") then
			createObject(descendant)
		end
	end)
	EspInterface.descendantRemoving = workspace.DescendantRemoving:Connect(function(descendant)
		if descendant:IsA("Model") then
			removeObject(descendant)
		end
	end)
...
function EspInterface.Unload()
	...
	EspInterface.descendantAdded:Disconnect()
	EspInterface.descendantRemoving:Disconnect()
	EspInterface._hasLoaded = false
end

function EspInterface.getWeapon(character)
	return "Unknown"
end
...
function EspInterface.getCharacter(character)
	return character
end
